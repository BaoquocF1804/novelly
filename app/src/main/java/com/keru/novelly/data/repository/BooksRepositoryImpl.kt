        ...
        try {
            ...
            fireStore.collection(path).document(id)
                .addSnapshotListener { snapshot, error ->
                    if (error != null) {
                        Log.e("BookTest", "Firestore error: ${error.message}", error)
                        value.tryEmit(Resource.Error(message = error.message ?: "Firestore error"))
                    } else if (snapshot == null) {
                        Log.e("BookTest", "Snapshot is null for bookId: $id")
                        value.tryEmit(Resource.Error(message = "Snapshot is null for bookId: $id"))
                    } else if (!snapshot.exists()) {
                        Log.e("BookTest", "Book not found for bookId: $id")
                        value.tryEmit(Resource.Error(message = "Book not found for bookId: $id"))
                    } else {
                        try {
                            val book = snapshot.toObject(Book::class.java)
                            if (book == null) {
                                Log.e("BookTest", "Failed to parse Book object for bookId: $id")
                                value.tryEmit(Resource.Error(message = "Failed to parse Book object for bookId: $id"))
                            } else {
                                Log.d("BookTest", "Book loaded successfully for bookId: $id")
                                value.tryEmit(Resource.Success(data = book))
                            }
                        } catch (e: Exception) {
                            Log.e("BookTest", "Exception when parsing Book object for bookId: $id", e)
                            value.tryEmit(Resource.Error(message = e.message ?: "Exception when parsing Book object"))
                        }
                    }
                }
        } catch (e: FirebaseFirestoreException) {
            Log.e("BookTest", "FirestoreException: $e", e)
            value.tryEmit(
